...
/**
 * A collection of various utility methods that are helpful for working with the Clipcoin protocol.
 * To enable debug logging from the library, run with -Dclipcoinj.logging=true on your command line.
 */
public class Utils {
    ...
    /** The string that prefixes all text messages signed using Clipcoin keys. */
    public static final String CLIPCOIN_SIGNED_MESSAGE_HEADER = "Clipcoin Signed Message:\n";

    // TODO: Replace this nanocoins business with something better.

    /**
     * How many "nanocoins" there are in a Clipcoin.
     * <p/>
     * A nanocoin is the smallest unit that can be transferred using Clipcoin.
    ...
    /**
     * How many "nanocoins" there are in 0.01 Clipcoins.
     * <p/>
     * A nanocoin is the smallest unit that can be transferred using Clipcoin.
     * The term nanocoin is very misleading, though, because there are only 100 million
     * of them in a coin (whereas one would expect 1 billion).
     ...
     * standard procedure in Clipcoin. The resulting hash is in big endian form.
     */
     ...
     */
    public static String clipcoinValueToFriendlyString(BigInteger value) {
     ...
     */
    public static String clipcoinValueToPlainString(BigInteger value) {
        if (value == null) {
     ...
     *
     * <tt><p>[24] "Clipcoin Signed Message:\n" [message.length as a varint] message</p></tt>
     */
    public static byte[] formatMessageForSigning(String message) {
        VarInt size = new VarInt(message.length());
        int totalSize = 1 + CLIPCOIN_SIGNED_MESSAGE_HEADER.length() + size.getSizeInBytes() + message.length();
        byte[] result = new byte[totalSize];
        int cursor = 0;
        result[cursor++] = (byte) CLIPCOIN_SIGNED_MESSAGE_HEADER.length();
        byte[] bytes = CLIPCOIN_SIGNED_MESSAGE_HEADER.getBytes(Charset.forName("UTF-8"));
